...
test-validation: generate fmt vet manifests  ## Run validation tests
	...
	go test --tags=validationtests -v ./test/e2e/validation -junit $(VALIDATION_TESTS_REPORTS_PATH) -report $(VALIDATION_TESTS_REPORTS_PATH)

.PHONY: test-functional
test-functional: generate fmt vet manifests  ## Run functional tests
	rm -rf ${TESTS_REPORTS_PATH}
	mkdir -p ${TESTS_REPORTS_PATH}
	go test -timeout 20m --tags=e2etests -v ./test/e2e/functional -junit $(TESTS_REPORTS_PATH) -report $(TESTS_REPORTS_PATH)
...
generate-daemon-manifest: ## Generate DaemonSet manifest.
	...
	PATH="$(LOCALBIN):$$PATH" hack/generate-daemon-manifest.sh
